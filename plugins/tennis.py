...
for match in tourney['match_data']:
    ...
    if '/' in first_name:
        if ('Game Set and Match' in match['info']) or ('Winners' in match['info']):
            if 'Walkover' in match['info']:
                if first_name.upper().split()[1] in match['info']:
                    d = f'| {first_name} d. {second_name}: w/o'
                else:
                    d = f'| {second_name} d. {first_name}: w/o'
                ...
                d = f'| {first_name} d. {second_name}: {first_set_num[0]}-{second_set_num[0]}'
                try:
                ...
                d = f'| {second_name} d. {first_name}: {second_set_num[0]}-{first_set_num[0]}'
                try:
        ...
        if ('Game Set and Match' in match['info']) or ('Winner' in match['info']):
            if 'Walkover' in match['info']:
                if first_name.upper().split()[1] in match['info']:
                    s = f'| {first_name} d. {second_name}: w/o'
                else:
                    s = f'| {second_name} d. {first_name}: w/o'
            elif (first_set_num[0] > second_set_num[0] and first_set_num[1] > second_set_num[1]) or (
                            first_set_num[1] > second_set_num[1] and first_set_num[2] > second_set_num[2]) or (
                            first_set_num[0] > second_set_num[0] and first_set_num[2] > second_set_num[2]):
                s = f'| {first_name} d. {second_name}: {first_set_num[0]}-{second_set_num[0]}'
                try:
                ...
                s = f'| {second_name} d. {first_name}: {second_set_num[0]}-{first_set_num[0]}'
                try:
